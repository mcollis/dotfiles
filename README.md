...
directory is an independent Stow package. Package contents use the `dot-`
prefix, which Stow maps to hidden target paths through `.stowrc`.
...
See [the agent setup guide](agents/dot-agents/README.md) before installing
...
`.stowrc` sets `--dotfiles`, `--no-folding`, and `--target=~`.
