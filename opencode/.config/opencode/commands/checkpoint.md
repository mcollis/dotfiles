...
2. **Merge, don't overwrite.** If a checkpoint already exists, combine what's already there with what's new this session — keep prior sections that are still valid, check off completed Progress items, and layer in new Decisions/Findings/Changes/Verification/References/Blockers/Next steps. Never regress a richer stored checkpoint down to just this session's (possibly thinner, e.g. post-compaction) context.
3. Gather/update the sections: **Status**, **Goal**, **Outcome**, **Decisions**, **Findings**, **Plan**, **Progress** (a Markdown task list), **Blockers**, **Next steps**, **Changes**, **Verification**, **References**, as structured Markdown.
