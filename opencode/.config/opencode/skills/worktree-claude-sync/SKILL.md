...
1. Run `"$HOME/.agents/plugins/ex/skills/worktree-claude-sync/run.sh" --dry-run` to see EDIT/MIRROR lines without changing anything.
...
3. After resolving EDITs, re-run `"$HOME/.agents/plugins/ex/skills/worktree-claude-sync/run.sh"` (no flags) to apply the mirror.
...
`"$HOME/.agents/plugins/ex/skills/worktree-claude-sync/cleanup.sh"` and
`"$HOME/.agents/plugins/ex/skills/worktree-claude-sync/unmigrate-hook.sh"`.
